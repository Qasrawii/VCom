...
const CompanyScreen = ({ navigation, route }) => {
  ...
  const [company, setCompany] = useState(route.params.item)
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} style={{ backgroundColor: colors.primary20 }}>
      ...
      <View style={{ backgroundColor: colors.white, paddingVertical: 30, paddingHorizontal: 20, borderTopRightRadius: 30, borderTopLeftRadius: 30 }}>
        ...
        <TouchableOpacity onPress={() =>auth().currentUser._user.uid!=undefined? navigation.navigate("ChatScreen", { resId: company.key, name: company.name }):null} style={{ flex: 1, borderWidth: 1, borderColor: colors.primary, padding: 10, borderRadius: 10, marginVertical: 15, justifyContent: 'center', alignItems: 'center' }}>
